...
def apply_match_result(*, match, winner):
    if not winner:
        raise ValueError('Winner is required for match results.')
    ...
    stats_a.save(update_fields=['elo', 'losses', 'matches_played', 'modified_date', 'wins'])
    stats_b.save(update_fields=['elo', 'losses', 'matches_played', 'modified_date', 'wins'])
