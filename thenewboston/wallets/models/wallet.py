...
class Wallet(AdjustableTimestampsModel):
    ...
    def change_balance(self, amount, save=True, should_stream=True, should_adjust_timestamps=True):
        if (new_balance := self.balance + amount) < 0:
    ...
    def save(self, *args, should_stream=False, should_adjust_timestamps=True, **kwargs):
        if self.has_changed('balance') and should_stream:
        ...
        return super().save(
            *args, should_adjust_timestamps=should_adjust_timestamps, **kwargs
        )  # return value for forward compatibility
